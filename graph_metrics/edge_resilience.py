...
def construct_auxiliary_graph(graph: dict, service: str) -> tuple[nx.Graph, str]:
    ...
    for u,v in G.edges:
        if (u in nodes_providing_service) and (v in nodes_providing_service):
            G.remove_edge(u,v)
        elif (u is new_node) and (v in nodes_providing_service):
...
def get_min_sv_edge_cutset(s: str, t: str, G: nx.Graph) -> float:
   cut_weight = nx.minimum_cut_value(G, s, t, capacity='weight')
   print(f"Min edge S-V cutset for {s}, {t} is {cut_weight}")
   return cut_weight
...
for graph in graphs:
    ...
    print(f"Graph edge resilience for {graph["name"]} is {graph_edge_resilience}")